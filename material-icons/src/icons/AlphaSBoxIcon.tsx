import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const AlphaSBoxIcon: FC<IconInterface> = function AlphaSBoxIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `AlphaSBoxIcon ${className}`;
      return (
        <Icon alt="AlphaSBox" className={classNames} {...propsRest}>
          <path d="M11,7A2,2 0 0,0 9,9V11A2,2 0 0,0 11,13H13V15H9V17H13A2,2 0 0,0 15,15V13A2,2 0 0,0 13,11H11V9H15V7H11M5,3H19A2,2 0 0,1 21,5V19A2,2 0 0,1 19,21H5A2,2 0 0,1 3,19V5A2,2 0 0,1 5,3Z" />

        </Icon>
      );
    };
    AlphaSBoxIcon.displayName = 'AlphaSBoxIcon';
      