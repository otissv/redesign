import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const BatteryAlertIcon: FC<IconInterface> = function BatteryAlertIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `BatteryAlertIcon ${className}`;
      return (
        <Icon alt="BatteryAlert" className={classNames} {...propsRest}>
          <path d="M13,14H11V9H13M13,18H11V16H13M16.67,4H15V2H9V4H7.33A1.33,1.33 0 0,0 6,5.33V20.67C6,21.4 6.6,22 7.33,22H16.67A1.33,1.33 0 0,0 18,20.67V5.33C18,4.6 17.4,4 16.67,4Z" />

        </Icon>
      );
    };
    BatteryAlertIcon.displayName = 'BatteryAlertIcon';
      