import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const CheckUnderlineCircleOutlineIcon: FC<IconInterface> = function CheckUnderlineCircleOutlineIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `CheckUnderlineCircleOutlineIcon ${className}`;
      return (
        <Icon alt="CheckUnderlineCircleOutline" className={classNames} {...propsRest}>
          <path d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M12,20C7.59,20 4,16.41 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20,12C20,16.41 16.41,20 12,20M7,15H17V17H7V15M10.3,11.2L8.4,9.3L7,10.7L10.3,14L17,7.3L15.6,5.9L10.3,11.2Z" />

        </Icon>
      );
    };
    CheckUnderlineCircleOutlineIcon.displayName = 'CheckUnderlineCircleOutlineIcon';
      