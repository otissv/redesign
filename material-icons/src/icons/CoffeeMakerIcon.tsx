import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const CoffeeMakerIcon: FC<IconInterface> = function CoffeeMakerIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `CoffeeMakerIcon ${className}`;
      return (
        <Icon alt="CoffeeMaker" className={classNames} {...propsRest}>
          <path d="M21 1V4H7V21H21V23H2V3C2 1.89 2.89 1 4 1H21M22 13V15C22 16.11 21.11 17 20 17H18C18 20 15 20 14 20S10 20 10 17V11H20C21.11 11 22 11.9 22 13M20 13H18V15H20V13M17 9L19 5H9L11 9H17Z" />

        </Icon>
      );
    };
    CoffeeMakerIcon.displayName = 'CoffeeMakerIcon';
      