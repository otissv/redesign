import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const FridgeBottomIcon: FC<IconInterface> = function FridgeBottomIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `FridgeBottomIcon ${className}`;
      return (
        <Icon alt="FridgeBottom" className={classNames} {...propsRest}>
          <path d="M7,2A2,2 0 0,0 5,4V19A2,2 0 0,0 7,21V22H9V21H15V22H17V21A2,2 0 0,0 19,19V4A2,2 0 0,0 17,2H7M8,6H10V8H8V6M7,11H17V19H7V11M8,12V15H10V12H8Z" />

        </Icon>
      );
    };
    FridgeBottomIcon.displayName = 'FridgeBottomIcon';
      