import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const HexagramIcon: FC<IconInterface> = function HexagramIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `HexagramIcon ${className}`;
      return (
        <Icon alt="Hexagram" className={classNames} {...propsRest}>
          <path d="M14.5,7.66L20.64,6.97L17,12L20.68,16.97L14.5,16.32L12.03,22L9.5,16.34L3.36,17.03L7,12L3.32,7.03L9.5,7.68L11.97,2L14.5,7.66Z" />

        </Icon>
      );
    };
    HexagramIcon.displayName = 'HexagramIcon';
      