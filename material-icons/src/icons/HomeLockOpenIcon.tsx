import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const HomeLockOpenIcon: FC<IconInterface> = function HomeLockOpenIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `HomeLockOpenIcon ${className}`;
      return (
        <Icon alt="HomeLockOpen" className={classNames} {...propsRest}>
          <path d="M12,3L2,12H5V20H19V12H22L12,3M12,8A3,3 0 0,1 15,11H13A1,1 0 0,0 12,10A1,1 0 0,0 11,11V13H16V17H8V13H9V11A3,3 0 0,1 12,8Z" />

        </Icon>
      );
    };
    HomeLockOpenIcon.displayName = 'HomeLockOpenIcon';
      