import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const IframeOutlineIcon: FC<IconInterface> = function IframeOutlineIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `IframeOutlineIcon ${className}`;
      return (
        <Icon alt="IframeOutline" className={classNames} {...propsRest}>
          <path d="M9.5,8.5L11,10L8,13L11,16L9.5,17.5L5,13L9.5,8.5M14.5,17.5L13,16L16,13L13,10L14.5,8.5L19,13L14.5,17.5M21,2H3A2,2 0 0,0 1,4V20A2,2 0 0,0 3,22H21A2,2 0 0,0 23,20V4A2,2 0 0,0 21,2M21,20H3V6H21V20Z" />

        </Icon>
      );
    };
    IframeOutlineIcon.displayName = 'IframeOutlineIcon';
      