import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const ImageAreaCloseIcon: FC<IconInterface> = function ImageAreaCloseIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `ImageAreaCloseIcon ${className}`;
      return (
        <Icon alt="ImageAreaClose" className={classNames} {...propsRest}>
          <path d="M12,23L8,19H16L12,23M20,3A2,2 0 0,1 22,5V15A2,2 0 0,1 20,17H4A2,2 0 0,1 2,15V5A2,2 0 0,1 4,3H20M5,14H19L14.5,8L11,12.5L8.5,9.5L5,14Z" />

        </Icon>
      );
    };
    ImageAreaCloseIcon.displayName = 'ImageAreaCloseIcon';
      