import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const LinkPlusIcon: FC<IconInterface> = function LinkPlusIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `LinkPlusIcon ${className}`;
      return (
        <Icon alt="LinkPlus" className={classNames} {...propsRest}>
          <path d="M7,7H11V9H7A3,3 0 0,0 4,12A3,3 0 0,0 7,15H11V17H7A5,5 0 0,1 2,12A5,5 0 0,1 7,7M17,7A5,5 0 0,1 22,12H20A3,3 0 0,0 17,9H13V7H17M8,11H16V13H8V11M17,12H19V15H22V17H19V20H17V17H14V15H17V12Z" />

        </Icon>
      );
    };
    LinkPlusIcon.displayName = 'LinkPlusIcon';
      