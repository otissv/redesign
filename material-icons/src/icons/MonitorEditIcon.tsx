import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const MonitorEditIcon: FC<IconInterface> = function MonitorEditIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `MonitorEditIcon ${className}`;
      return (
        <Icon alt="MonitorEdit" className={classNames} {...propsRest}>
          <path d="M21 2H3C1.9 2 1 2.9 1 4V16C1 17.11 1.9 18 3 18H10V20H8V22H16V20H14V18H21C22.11 18 23 17.11 23 16V4C23 2.9 22.11 2 21 2M21 16H3V4H21V16M15.84 8.2L14.83 9.21L12.76 7.18L13.77 6.16C13.97 5.95 14.31 5.94 14.55 6.16L15.84 7.41C16.05 7.62 16.06 7.96 15.84 8.2M8 11.91L12.17 7.72L14.24 9.8L10.08 14H8V11.91Z" />

        </Icon>
      );
    };
    MonitorEditIcon.displayName = 'MonitorEditIcon';
      