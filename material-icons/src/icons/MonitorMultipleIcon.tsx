import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const MonitorMultipleIcon: FC<IconInterface> = function MonitorMultipleIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `MonitorMultipleIcon ${className}`;
      return (
        <Icon alt="MonitorMultiple" className={classNames} {...propsRest}>
          <path d="M22,17V7H6V17H22M22,5A2,2 0 0,1 24,7V17C24,18.11 23.1,19 22,19H16V21H18V23H10V21H12V19H6C4.89,19 4,18.11 4,17V7A2,2 0 0,1 6,5H22M2,3V15H0V3A2,2 0 0,1 2,1H20V3H2Z" />

        </Icon>
      );
    };
    MonitorMultipleIcon.displayName = 'MonitorMultipleIcon';
      