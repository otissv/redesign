import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const MusicCircleOutlineIcon: FC<IconInterface> = function MusicCircleOutlineIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `MusicCircleOutlineIcon ${className}`;
      return (
        <Icon alt="MusicCircleOutline" className={classNames} {...propsRest}>
          <path d="M16,9H13V14.5A2.5,2.5 0 0,1 10.5,17A2.5,2.5 0 0,1 8,14.5A2.5,2.5 0 0,1 10.5,12C11.07,12 11.58,12.19 12,12.5V7H16V9M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4Z" />

        </Icon>
      );
    };
    MusicCircleOutlineIcon.displayName = 'MusicCircleOutlineIcon';
      