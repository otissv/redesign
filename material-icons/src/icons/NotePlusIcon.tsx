import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const NotePlusIcon: FC<IconInterface> = function NotePlusIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `NotePlusIcon ${className}`;
      return (
        <Icon alt="NotePlus" className={classNames} {...propsRest}>
          <path d="M14,10H19.5L14,4.5V10M5,3H15L21,9V19A2,2 0 0,1 19,21H5C3.89,21 3,20.1 3,19V5C3,3.89 3.89,3 5,3M9,18H11V15H14V13H11V10H9V13H6V15H9V18Z" />

        </Icon>
      );
    };
    NotePlusIcon.displayName = 'NotePlusIcon';
      