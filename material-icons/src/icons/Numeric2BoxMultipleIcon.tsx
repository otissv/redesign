import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const Numeric2BoxMultipleIcon: FC<IconInterface> = function Numeric2BoxMultipleIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `Numeric2BoxMultipleIcon ${className}`;
      return (
        <Icon alt="Numeric2BoxMultiple" className={classNames} {...propsRest}>
          <path d="M3,5V21H19V23H3A2,2 0 0,1 1,21V5H3M17,13H13V11H15A2,2 0 0,0 17,9V7C17,5.89 16.1,5 15,5H11V7H15V9H13A2,2 0 0,0 11,11V15H17V13M21,1A2,2 0 0,1 23,3V17A2,2 0 0,1 21,19H7A2,2 0 0,1 5,17V3A2,2 0 0,1 7,1H21Z" />

        </Icon>
      );
    };
    Numeric2BoxMultipleIcon.displayName = 'Numeric2BoxMultipleIcon';
      