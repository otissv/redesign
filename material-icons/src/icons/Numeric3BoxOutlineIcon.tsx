import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const Numeric3BoxOutlineIcon: FC<IconInterface> = function Numeric3BoxOutlineIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `Numeric3BoxOutlineIcon ${className}`;
      return (
        <Icon alt="Numeric3BoxOutline" className={classNames} {...propsRest}>
          <path d="M15,15V13.5A1.5,1.5 0 0,0 13.5,12A1.5,1.5 0 0,0 15,10.5V9C15,7.89 14.1,7 13,7H9V9H13V11H11V13H13V15H9V17H13A2,2 0 0,0 15,15M19,19H5V5H19M19,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3Z" />

        </Icon>
      );
    };
    Numeric3BoxOutlineIcon.displayName = 'Numeric3BoxOutlineIcon';
      