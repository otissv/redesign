import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const Numeric6BoxMultipleOutlineIcon: FC<IconInterface> = function Numeric6BoxMultipleOutlineIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `Numeric6BoxMultipleOutlineIcon ${className}`;
      return (
        <Icon alt="Numeric6BoxMultipleOutline" className={classNames} {...propsRest}>
          <path d="M13,11H15V13H13M13,15H15A2,2 0 0,0 17,13V11C17,9.89 16.1,9 15,9H13V7H17V5H13A2,2 0 0,0 11,7V13C11,14.11 11.9,15 13,15M21,17H7V3H21M21,1H7A2,2 0 0,0 5,3V17A2,2 0 0,0 7,19H21A2,2 0 0,0 23,17V3A2,2 0 0,0 21,1M3,5H1V21A2,2 0 0,0 3,23H19V21H3V5Z" />

        </Icon>
      );
    };
    Numeric6BoxMultipleOutlineIcon.displayName = 'Numeric6BoxMultipleOutlineIcon';
      