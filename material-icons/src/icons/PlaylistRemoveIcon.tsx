import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const PlaylistRemoveIcon: FC<IconInterface> = function PlaylistRemoveIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `PlaylistRemoveIcon ${className}`;
      return (
        <Icon alt="PlaylistRemove" className={classNames} {...propsRest}>
          <path d="M2,6V8H14V6H2M2,10V12H11V10H2M14.17,10.76L12.76,12.17L15.59,15L12.76,17.83L14.17,19.24L17,16.41L19.83,19.24L21.24,17.83L18.41,15L21.24,12.17L19.83,10.76L17,13.59L14.17,10.76M2,14V16H11V14H2Z" />

        </Icon>
      );
    };
    PlaylistRemoveIcon.displayName = 'PlaylistRemoveIcon';
      