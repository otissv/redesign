import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign/ui-core";
    export const RepeatOffIcon: FC<IconInterface> = function RepeatOffIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `RepeatOffIcon ${className}`;
      return (
        <Icon alt="RepeatOff" className={classNames} {...propsRest}>
          <path d="M2,5.27L3.28,4L20,20.72L18.73,22L15.73,19H7V22L3,18L7,14V17H13.73L7,10.27V11H5V8.27L2,5.27M17,13H19V17.18L17,15.18V13M17,5V2L21,6L17,10V7H8.82L6.82,5H17Z" />

        </Icon>
      );
    };
    RepeatOffIcon.displayName = 'RepeatOffIcon';
      