import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const ShredderIcon: FC<IconInterface> = function ShredderIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `ShredderIcon ${className}`;
      return (
        <Icon alt="Shredder" className={classNames} {...propsRest}>
          <path d="M6,3V7H8V5H16V7H18V3H6M5,8A3,3 0 0,0 2,11V17H5V14H19V17H22V11A3,3 0 0,0 19,8H5M18,10A1,1 0 0,1 19,11A1,1 0 0,1 18,12A1,1 0 0,1 17,11A1,1 0 0,1 18,10M7,16V21H9V16H7M11,16V20H13V16H11M15,16V21H17V16H15Z" />

        </Icon>
      );
    };
    ShredderIcon.displayName = 'ShredderIcon';
      