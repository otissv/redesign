import React, { FC } from 'react';
    import { Icon, IconInterface } from "@redesign-system/ui-core";
    export const StockingIcon: FC<IconInterface> = function StockingIcon({
      className,
      ...propsRest
    }) {
      const classNames =  `StockingIcon ${className}`;
      return (
        <Icon alt="Stocking" className={classNames} {...propsRest}>
          <path d="M17,2A2,2 0 0,1 19,4V7A2,2 0 0,1 17,9V17C17,17.85 16.5,18.57 15.74,18.86L9.5,21.77C8.5,22.24 7.29,21.81 6.83,20.81L6,19C5.5,18 5.95,16.8 6.95,16.34L10,14.91V9A2,2 0 0,1 8,7V4A2,2 0 0,1 10,2H17M10,4V7H17V4H10Z" />

        </Icon>
      );
    };
    StockingIcon.displayName = 'StockingIcon';
      