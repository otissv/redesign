import React, { FC, useMemo } from 'react';

import { IconsInterface } from '../icons.types';
import { Icon } from '../Icon';

export const CodeEqualIcon: FC<IconsInterface> = function CodeEqualIcon({
  className,
  ...propsRest
}) {

  const classNames = useMemo(() => `CodeEqualIcon ${className}`, [className]);

  return (
    <Icon alt="CodeEqual" className={classNames} {...propsRest}>
      <path d="M6,13H11V15H6M13,13H18V15H13M13,9H18V11H13M6,9H11V11H6M5,3C3.89,3 3,3.9 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3H5Z" />
    </Icon>
  );
};

CodeEqualIcon.displayName = 'CodeEqualIcon';  
  