import React, { FC, useMemo } from 'react';

import { IconsInterface } from '../icons.types';
import { Icon } from '../Icon';

export const CodeLessThanOrEqualIcon: FC<IconsInterface> = function CodeLessThanOrEqualIcon({
  className,
  ...propsRest
}) {

  const classNames = useMemo(() => `CodeLessThanOrEqualIcon ${className}`, [className]);

  return (
    <Icon alt="CodeLessThanOrEqual" className={classNames} {...propsRest}>
      <path d="M13,13H18V15H13M13,9H18V11H13M10.09,7.41L11.5,8.82L8.32,12L11.5,15.18L10.09,16.6L5.5,12M5,3C3.89,3 3,3.9 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3H5Z" />
    </Icon>
  );
};

CodeLessThanOrEqualIcon.displayName = 'CodeLessThanOrEqualIcon';  
  