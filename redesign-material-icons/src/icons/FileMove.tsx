import React, { FC, useMemo } from 'react';

import { IconsInterface } from '../icons.types';
import { Icon } from '../Icon';

export const FileMoveIcon: FC<IconsInterface> = function FileMoveIcon({
  className,
  ...propsRest
}) {

  const classNames = useMemo(() => `FileMoveIcon ${className}`, [className]);

  return (
    <Icon alt="FileMove" className={classNames} {...propsRest}>
      <path d="M14,17H18V14L23,18.5L18,23V20H14V17M13,9H18.5L13,3.5V9M6,2H14L20,8V12.34C19.37,12.12 18.7,12 18,12A6,6 0 0,0 12,18C12,19.54 12.58,20.94 13.53,22H6C4.89,22 4,21.1 4,20V4A2,2 0 0,1 6,2Z" />
    </Icon>
  );
};

FileMoveIcon.displayName = 'FileMoveIcon';  
  