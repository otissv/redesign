import React, { FC, useMemo } from 'react';

import { IconsInterface } from '../icons.types';
import { Icon } from '../Icon';

export const FileRefreshOutlineIcon: FC<IconsInterface> = function FileRefreshOutlineIcon({
  className,
  ...propsRest
}) {

  const classNames = useMemo(() => `FileRefreshOutlineIcon ${className}`, [className]);

  return (
    <Icon alt="FileRefreshOutline" className={classNames} {...propsRest}>
      <path d="M18 14.5C19.11 14.5 20.11 14.95 20.83 15.67L22 14.5V18.5H18L19.77 16.73C19.32 16.28 18.69 16 18 16C16.62 16 15.5 17.12 15.5 18.5C15.5 19.88 16.62 21 18 21C18.82 21 19.54 20.61 20 20H21.71C21.12 21.47 19.68 22.5 18 22.5C15.79 22.5 14 20.71 14 18.5C14 16.29 15.79 14.5 18 14.5M14 2L20 8V12.17C19.5 12.06 19 12 18.5 12L18 12V9H13V4H6V20H12.17C12.34 20.72 12.63 21.39 13 22H6C4.9 22 4 21.1 4 20V4C4 2.9 4.9 2 6 2H14Z" />
    </Icon>
  );
};

FileRefreshOutlineIcon.displayName = 'FileRefreshOutlineIcon';  
  