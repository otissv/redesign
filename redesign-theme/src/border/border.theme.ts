  ...
  const defaults: BorderInterface = {
    ...
    ...Object.keys(color).reduce((previous, key) => {
      ...
      return {
        ...
        [key]: color_key,
        [`${toCamel(
          `thick${toUpperFirst(key)}`
        )}`]: `${BORDER_THICK_WIDTH} ${BORDER_STYLE} ${color_key}`,
        [`${toCamel(
          `thin${toUpperFirst(key)}`
        )}`]: `${BORDER_THIN_WIDTH} ${BORDER_STYLE} ${color_key}`,
        [`${toCamel(
          `thickDashed${toUpperFirst(key)}`
        )}`]: `${BORDER_THICK_WIDTH} ${BORDER_DASH} ${color_key}`,
        [`${toCamel(
          `thinDashed${toUpperFirst(key)}`
        )}`]: `${BORDER_THIN_WIDTH} ${BORDER_DASH} ${color_key}`,
      };
    }, initialBorders),
  };
