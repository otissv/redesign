import { utilityTheme, isValidStringValue, isValidNumberValue, isValidUnitValue, getRadiuses, unitPositions, allPositions } from './utility.theme';
import { theme as testTheme, utilityTestTheme } from '../../testing/fixtures';
describe('utilityTheme', function() {
    it('margin - utility theme', function() {
        var theme = utilityTestTheme;
        var actual = utilityTheme(theme);
        expect(actual.margin && actual.margin('100%')).toEqual({
            margin: '100%'
        });
    ...
    it('padding - utility theme', function() {
        var theme = utilityTestTheme;
        var actual = utilityTheme(theme);
    ...
    //   // });
    // });
    it('can overwrite utility theme', function() {
        var theme = {
            utility: {
                radius: function(_value) {
                    return {
                        borderRadius: '99px'
                    };
                }
    ...
    it('can extend utility theme', function() {
        var theme = {
            utility: {
                test: 'test'
            }
    ...
    it('isValidStringValue returns true if valid string', function() {
        expect(isValidStringValue('test')).toBe(true);
    });
    it('isValidStringValue returns false if empty string', function() {
        expect(isValidStringValue('')).toBe(false);
    });
    it('isValidStringValue returns false number', function() {
        expect(isValidStringValue(1)).toBe(false);
    });
    it('isValidStringValue returns false if undefined', function() {
        expect(isValidStringValue(undefined)).toBe(false);
    });
    it('isValidStringValue returns false if null', function() {
        expect(isValidStringValue(null)).toBe(false);
    });
    it('isValidStringValue returns false if false', function() {
        expect(isValidStringValue(true)).toBe(false);
    });
    it('isValidStringValue returns false if true', function() {
        expect(isValidStringValue(false)).toBe(false);
    });
    it('isValidNumberValue returns true if valid number', function() {
        expect(isValidNumberValue(1)).toBe(true);
    });
    it('isValidNumberValue returns false empty string', function() {
        expect(isValidNumberValue('')).toBe(false);
    });
    it('isValidNumberValue returns false empty string', function() {
        expect(isValidNumberValue('test')).toBe(false);
    });
    it('isValidNumberValue returns false if undefined', function() {
        expect(isValidNumberValue(undefined)).toBe(false);
    });
    it('isValidNumberValue returns false if null', function() {
        expect(isValidNumberValue(null)).toBe(false);
    });
    it('isValidNumberValue returns false if false', function() {
        expect(isValidNumberValue(true)).toBe(false);
    });
    it('isValidNumberValue returns false if true', function() {
        expect(isValidNumberValue(false)).toBe(false);
    });
    it('isValidUnitValue returns %', function() {
        expect(isValidUnitValue('%')).toBe(true);
    });
    it('isValidUnitValue returns px', function() {
        expect(isValidUnitValue('px')).toBe(true);
    });
    it('isValidUnitValue returns rem', function() {
        expect(isValidUnitValue('rem')).toBe(true);
    });
    it('isValidUnitValue returns px', function() {
        expect(isValidUnitValue('px')).toBe(true);
    });
    it('isValidUnitValue returns false if invalid', function() {
    ...
    it('unitPositions margin', function() {
        var getStyle = unitPositions({
            unit: testTheme.unit,
            style: 'margin'
        }).margin;
        expect(getStyle('100%')).toEqual({
            margin: '100%'
        });
        expect(getStyle('default')).toEqual({
            margin: '24px'
        });
        expect(getStyle(10)).toEqual({
            margin: '128px'
        });
